                ...
                <TabsContent value='overview' className='space-y-4'>
                    <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-7'>
                        ...
                        <Card className='lg:col-span-3'>
                            ...
                            <CardContent>
                                {loading
                                    ? <div className='space-y-3'>{Array(4).fill(0).map((_, i) => <Skeleton key={i}
                                                                                                           className='h-12 w-full'/>)}</div>
                    ...
                    <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-7'>
                        <Card className='lg:col-span-3'>
                            ...
                            <CardContent>
                                {loading
                                    ? <div className='space-y-3'>{Array(3).fill(0).map((_, i) => <Skeleton key={i}
                                                                                                           className='h-16 w-full'/>)}</div>
                        ...
                        <Card className='lg:col-span-4'>
                            ...
                            <CardContent>
                                {loading
                                    ? <div className='space-y-3'>{Array(5).fill(0).map((_, i) => <Skeleton key={i}
                                                                                                           className='h-8 w-full'/>)}</div>
                                    : analytics?.top_products.length === 0
                                        ? <p className='text-sm text-muted-foreground text-center py-4'>Нет данных о
                                            продажах</p>
                                        : (
                                            <div className='space-y-4'>
                                                {analytics?.top_products.map((product, i) => (
                                                    <div key={i} className='flex items-center'>
                                                        <div
                                                            className='w-6 text-sm text-muted-foreground font-medium'>{i + 1}.
                                                        </div>
                                                        <div className='flex-1 flex justify-between items-center'>
                                                            <div
                                                                className='font-medium text-sm truncate max-w-[180px]'>{product.name}</div>
                                                            <div className='flex items-center gap-4 shrink-0'>
                                                                <div
                                                                    className='text-sm text-muted-foreground'>{product.quantity} шт.
                                                                </div>
                                                                <div
                                                                    className='font-medium text-sm'>{formatPrice(product.revenue)}</div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                ))}
                    ...
                    <Card>
                        ...
                        <CardContent>
                            {loading
                                ...
                                : analytics?.top_products.length === 0
                                    ? <p className='text-sm text-muted-foreground text-center py-8'>Нет данных о
                                        продажах</p>
                                    : (
                                        <div className='rounded-md border'>
                                            <div
                                                className='grid grid-cols-12 gap-4 p-3 text-sm font-medium border-b text-muted-foreground'>
                                                ...
                                                {analytics?.top_products.map((product, i) => {
                                                    ...
                                                    return (
                                                        <div key={i}
                                                            ...
                                                            <div
                                                                className='col-span-1 text-muted-foreground'>{i + 1}</div>
                                                            <div className='col-span-5'>
                                                                <div
                                                                    className='font-medium truncate'>{product.name}</div>
                                                                <div
                                                            ...
                                                            <div
                                                                className='col-span-3 text-right'>{product.quantity} шт.
                                                            </div>
