							...
							<CardContent className='space-y-4'>
								...
								<div className='space-y-2'>
									...
									<div className='flex items-center space-x-2'>
										...
										<Label htmlFor='lowStock'>Уведомлять по email, когда запасы
											заканчиваются</Label>
									</div>
								</div>
							</CardContent>
								...
								<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
									{productData.images.map((image: string, index: number) => (
										<div key={index}
											...
											{index === 0 && (
												<div
													className='absolute bottom-2 left-2 bg-primary text-primary-foreground text-xs px-2 py-1 rounded'>
                          Главное изображение
												</div>
											)}
										</div>
